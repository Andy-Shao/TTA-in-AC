...
from ttt.lib.speech_commands.prepare_dataset import prepare_data, val_transforms as test_transforms, train_transforms, add_normalization
...
if __name__ == '__main__':
    ...
    print('calculate test dataset mean and standard deviation')
    test_transf = add_normalization(args=args, tsf_dict=test_transf, dataset=test_dataset)
    ...
    for corruption in args.corruptions:
        ...
        corrupted_tsf = test_transforms(args)
        print('calculate the corrupted dataset mean and standard deviation')
        corrupted_tsf = add_normalization(args=args, tsf_dict=corrupted_tsf, dataset=corrupted_dataset)

        print(f'{corruption} corrupted test')
        net, ext, head, ssh = load_model(args)
        ttl_weight_num = count_ttl_params(model=net) + count_ttl_params(model=ext) + count_ttl_params(model=head)
        test_accu = inference(model=net, loader=corrupted_loader, test_transf=corrupted_tsf, device=args.device)
